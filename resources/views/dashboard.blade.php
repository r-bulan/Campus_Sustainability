<x-layouts.app :title="__('Dashboard')">
    ...
    <!-- Page header (keeps inside main so it won't be hidden) -->
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 class="text-3xl font-bold text-gray-900">Initiatives Dashboard</h1>
            <p class="mt-1 text-gray-500">Track, manage, and monitor sustainability initiatives.</p>
        </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        <!-- Success Message -->
        @if (session('success'))
            <div id="flash-success" class="max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
            <div class="text-xs uppercase tracking-wide text-gray-500">Total Initiatives</div>
            <div class="mt-2 text-3xl font-bold text-gray-900">{{ $initiativesCount }}</div>
        </div>

        <div class="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
            <div class="text-xs uppercase tracking-wide text-gray-500">Total Categories</div>
            <div class="mt-2 text-3xl font-bold text-gray-900">{{ $categoriesCount }}</div>
        </div>

        <div class="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
            <div class="text-xs uppercase tracking-wide text-gray-500">Total Impact Score</div>
            <div class="mt-2 text-3xl font-bold text-gray-900">{{ $totalImpact }}</div>
        </div>
    </div>
    ...
    <button id="open-modal-btn"
        ...
        <div class="hidden md:block bg-white rounded-xl border border-gray-100 shadow-sm mt-6 overflow-x-auto">
    <table class="min-w-full text-sm">
        <thead class="bg-gray-50 text-gray-600">
            <tr>
                ...
                <th class="px-4 py-3 text-left font-medium">Impact</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 transition">
                <td class="px-4 py-4">
                    <div class="font-semibold text-gray-900">{{ $initiative->title }}</div>
                    <div class="text-xs text-gray-500">{{ Str::limit($initiative->description, 80) }}</div>
                </td>

                <td class="px-4 py-4 text-gray-700">{{ optional($initiative->category)->name ?? '-' }}</td>
                <td class="px-4 py-4 text-gray-700">{{ $initiative->start_date }}</td>
                <td class="px-4 py-4 text-gray-700">{{ $initiative->end_date }}</td>

                <td class="px-4 py-4 font-semibold text-gray-900">{{ $initiative->impact_score }}</td>

                <td class="px-4 py-4">
                    <div class="flex gap-2">
                        <button class="edit-btn px-3 py-1.5 rounded-md bg-emerald-50 text-emerald-700 text-sm"
                            data-id="{{ $initiative->id }}"
                            data-title="{{ e($initiative->title) }}"
                            data-description="{{ e($initiative->description) }}"
                            data-category="{{ $initiative->category_id }}"
                            data-start="{{ $initiative->start_date }}"
                            data-end="{{ $initiative->end_date }}"
                            data-impact="{{ $initiative->impact_score }}">
                            Edit
                        </button>

                        <form method="POST"
                            action="{{ url('/initiatives/'.$initiative->id) }}"
                            onsubmit="return confirm('Delete this initiative?');">
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="px-3 py-1.5 rounded-md bg-red-50 text-red-600 text-sm">
                                Delete
                            </button>
                        </form>
                    </div>
...
@forelse ($initiatives ?? [] as $initiative)
    <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        <div class="flex justify-between items-start">
            <div class="min-w-0">
                <div class="font-semibold text-gray-900 truncate">{{ $initiative->title }}</div>
                <div class="text-xs text-gray-500 mt-1 truncate">
                    {{ Str::limit($initiative->description, 120) }}
                </div>
                <div class="mt-3 flex flex-wrap gap-2 text-xs text-gray-600">
                    <span class="bg-gray-100 px-2 py-1 rounded-full">
                        {{ optional($initiative->category)->name ?? 'Uncategorized' }}
                    </span>
                    <span class="bg-gray-100 px-2 py-1 rounded-full">
            ...
            <div class="text-right">
                <div class="text-xl font-bold text-gray-900">{{ $initiative->impact_score }}</div>
                <div class="text-xs text-gray-400">Impact</div>
            </div>
        </div>

        <div class="mt-4 flex flex-col gap-2">
            <button class="edit-btn w-full px-3 py-2 rounded-md bg-emerald-50 text-emerald-700 text-sm"
                data-id="{{ $initiative->id }}"
                data-title="{{ e($initiative->title) }}"
                data-description="{{ e($initiative->description) }}"
                data-category="{{ $initiative->category_id }}"
            ...
            <form method="POST" action="{{ url('/initiatives/'.$initiative->id) }}">
                @csrf @method('DELETE')
                <button type="submit" class="w-full px-3 py-2 rounded-md bg-red-50 text-red-600 text-sm">
                    Delete
                </button>
            </form>
        </div>
...
@empty
    <div class="text-center text-gray-500">No initiatives yet.</div>
@endforelse
</div>
